...
namespace CommunityLib
{
    public static class Pathfinding
    {
        ...
        public static Dictionary<Location, Location[]> getPathsFrom(Location locA, List<Func<Location[], Location, Unit, double>> pathfindingDelegates, List<Func<Location[], Location, Unit, List<Location>>> getNeighboursDelegates, Unit u = null, bool safeMove = false)
        {
            if (locA == null)
            {
                return null;
            ...
            foreach (var hook in ModCore.Get().HookRegistry.Delegate_onPopulatingPathfindingDelegates)
            {
                hook(locA, null, u, pathfindingDelegates, getNeighboursDelegates);
            }
            foreach (Hooks hook in ModCore.Get().GetRegisteredHooks())
            {
                hook.onPopulatingPathfindingDelegates(locA, null, u, pathfindingDelegates, getNeighboursDelegates);
            }
            ...
            for (int pass = 0; pass < 2; pass++)
            {
                ...
                while (i < 5 * start.map.locations.Count && paths.Count > 0)
                {
                    ...
                    if (currentLocation == end)
                    {
                        ...
                        {
                            if (!validityDelegate(pair.Value, currentLocation))
                            {
                                isValid = false;
                            }
                        }
