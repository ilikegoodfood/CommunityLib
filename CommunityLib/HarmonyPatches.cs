...
namespace CommunityLib
{
    public static class HarmonyPatches
    {
        ...
        private static void Patching()
        {
            ...
            harmony.Patch(original: AccessTools.Method(typeof(GraphicalMap), nameof(GraphicalMap.panTo), new Type[] { typeof(Hex) }), prefix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_Hex_Prefix)), postfix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_BulkPostfix)));
            harmony.Patch(original: AccessTools.Method(typeof(GraphicalMap), nameof(GraphicalMap.panTo), new Type[] { typeof(int), typeof(int), typeof(int) }), prefix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_Coordinate_Prefix)), postfix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_BulkPostfix)));
            harmony.Patch(original: AccessTools.Method(typeof(GraphicalMap), nameof(GraphicalMap.panTo), new Type[] { typeof(Unit) }), prefix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_Unit_Prefix)), postfix: new HarmonyMethod(patchType, nameof(GraphicalMap_panTo_BulkPostfix)));
        ...
        {
            __state = GraphicalMap.z != hex.z;
        }

        private static void GraphicalMap_panTo_BulkPostfix(bool __state)
        {
            if (__state)
            {
                foreach (ModKernel mod in EnumerateModsThatApplyGraphicalHexUpdate(World.staticMap))
        ...
        private static void GraphicalMap_panTo_Coordinate_Prefix(int panToZ, out bool __state)
        {
            __state = panToZ != GraphicalMap.z;
        }

        private static void GraphicalMap_panTo_Unit_Prefix(Unit unit, out bool __state)
        {
            __state = unit.location.hex.z != GraphicalMap.z;
        }
        ...
        private static IEnumerable<CodeInstruction> Task_PerformChallenge_turnTick_Transpiler(IEnumerable<CodeInstruction> codeInstructions, ILGenerator ilg)
        {
            ...
            for (int i = 0; i < instructionList.Count; i++)
            {
                if (targetIndex > 0)
                {
                    if (targetIndex == 1)
                    {
                        if (instructionList[i].opcode == OpCodes.Ldstr && (string)instructionList[i].operand == ": No Longer Valid")
                        {
                            instructionList[i].operand = ": No longer valid.";
                    ...
                    {
                        if (instructionList[i].opcode == OpCodes.Ldstr && (string)instructionList[i].operand == ": No Longer Valid for ")
                        {
                            instructionList[i].operand = ": No longer valid for ";
                    ...
                    {
                        if (instructionList[i].opcode == OpCodes.Ldc_I4_S)
                        {
        ...
        {
            if (__result && __instance.map.awarenessOfUnderground < 1.0)
            {
        ...
        // Nation Action Fixes
        private static void AN_RazeSubsettlement_getUtility_Postfix(Society soc, Person ruler, List<ReasonMsg> reasons, ref double __result)
        {
            if (soc.isDark())
            {
                double val = -soc.map.param.utility_soc_razeSubsettlementExternal;
                __result += val;
                reasons?.Add(new ReasonMsg("Base reluctance", val));
            }
        }
        ...
        // Defeatable Vinerva
        private static void P_Vinerva_HeartOfForest_cast_Postfix(P_Vinerva_HeartOfForest __instance)
        {
            if (__instance.map.overmind.god is God_Vinerva)
            {
                ModCore.Get().data.VinervaDefeatable = true;
            }
        }

        public static void God_Vinerva_turnTick_Postfix(God_Vinerva __instance)
        {
            if (!ModCore.opt_defeatableVinerva || __instance.map.overmind.god != __instance || __instance.map.overmind.victoryAchieved || __instance.map.overmind.endOfGameAchieved)
            {
                return;
            }

            if (__instance.hearts.Count == 0 && ModCore.Get().data.VinervaDefeatable)
            {
